...
# Reads a Burp Exported .xml file and converts it to wstalker csv to be imported with extensions Logger++ or Import Sitemap
def parse_burp_xml(input_file, output, import_sitemap=False):
    counter = 1
    csv_rows = []
    ...
    for item in root.findall("item"):
        ...
        if not import_sitemap:
            ...
            request_b64 = base64.b64encode(raw_request_bytes).decode()
            response_b64 = response_base64 if response_base64 else ""
    ...
    print(f"Saved {counter - 1} requests")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse Burp Suite XML export and convert it to wstalker CSV to import with Extensions Logger++ or ImportSitemap. Default: Logger++")
    parser.add_argument("-f", "--file", required=True, help="Path to Burp Suite .xml export file")
    parser.add_argument("-o", "--output", required=True, help="CSV in [request_base64,response_base64,method,url] format")
    parser.add_argument("-is", "--import_sitemap", type=bool, action=argparse.BooleanOptionalAction, help="ImportSitemap extension format: B64 Request, B64 Response, URL (no b64 URL safe)")

    args = parser.parse_args()
    parse_burp_xml(args.file, args.output, args.import_sitemap)
